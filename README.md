...
7. Run `pod install` in the `ios` folder from a terminal window.
  7.1 *From now on, use the xcworkspace generated by cocoapods to work on the native iOS files*.
8. Open the `xcworkspace` file that was generated by CocoaPods, located in the `ios` folder.
9. In `AppDelegate.m`, add the following with the other imports: `#import <PureSDK/Pure.h>`
    *Note: If 'React/RCTBundleURLProvider.h' or 'PureSDK/Pure.h can not be found, add it to your product schemes by doing the following: Navigate to `Product` ➜  `Schemes` ➜  `Manage Schemes`. Select `+` and add `RNPure` and `React` as target.*
