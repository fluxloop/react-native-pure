...
9. In `AppDelegate.m`, add the following with the other imports: `#import <PureSDK/Pure.h>` *Note: If 'React/RCTBundleURLProvider.h' or 'PureSDK/Pure.h can not be found, add it to your product schemes by doing the following: Navigate to `Product` ➜  `Schemes` ➜  `Manage Schemes`. Select `+` and add `RNPure` and `React` as target.*
